    ...
    <section ref={containerRef} className="relative w-full py-16 md:py-32 overflow-hidden bg-gradient-to-b from-[#050505] via-[#050a15] to-[#050505]">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 mb-20">
        <div className="tech-header text-center">
          ...
          <h2 className="text-[clamp(2.2rem,5vw,5rem)] text-white mb-2 md:mb-4 tracking-tight drop-shadow-md leading-tight" style={{ fontFamily: "var(--font-syne), sans-serif" }}>
            Tech Stream
          </h2>
          <p className="text-white/50 text-[clamp(0.75rem,2vw,1rem)] font-light tracking-[0.2em] uppercase" style={{ fontFamily: "var(--font-dm-sans), sans-serif" }}>
            Built with modern technologies
          </p>
        </div>
      </div>

      {/* 3 Infinite Scrolling Ticker Layers - CSS-only marquee */}
      <div ref={tickerRef} className="relative flex flex-col gap-6 overflow-hidden w-[400%] md:w-[200%] mx-auto group py-4">
        ...
        {techRows.map((row, rowIndex) => (
          ...
          >
            {/* 4-copy repetition with 5 items per row */}
            {[...row, ...row, ...row, ...row].map((tech, i) => (
              <div
                key={`${rowIndex}-${i}`}
                className="ticker-item inline-flex items-center gap-3 px-8 py-4 rounded-full border border-white/5 bg-[#0a0a0a] md:bg-white/[0.03] text-white/80 hover:border-white/20 hover:bg-white/[0.06] hover:text-white md:hover:-translate-y-1 transition-all duration-300 font-sans shadow-lg cursor-default"
              >
                <div className="opacity-90 group-hover:drop-shadow-[0_0_8px_currentColor] transition-all duration-300">{tech.icon}</div>
